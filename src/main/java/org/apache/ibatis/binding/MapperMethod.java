...
public class MapperMethod {
  ...
  public static class MethodSignature {
    ...
    public MethodSignature(Configuration configuration, Method method) {
      ...
      //判断method方法的参数中是否存在@param注解
      this.hasNamedParameters = hasNamedParams(method);
      //以下重复循环2遍调用getUniqueParamIndex，是不是降低效率了
      //记下RowBounds是第几个参数
      this.rowBoundsIndex = getUniqueParamIndex(method, RowBounds.class);
      //记下ResultHandler是第几个参数    注： method方法中标记一下ResultHandler是第几个参数，将位置索引进行返回
      this.resultHandlerIndex = getUniqueParamIndex(method, ResultHandler.class);
      /**
       * getParams(method, this.hasNamedParameters) 此步骤是获取method的所有参数列表，
       * 返回的形式如 0 : @param注解的值（value）
       * 再用unmodifiableSortedMap 封装一下
       */
